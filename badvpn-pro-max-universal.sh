#!/bin/bash
# BADVPN PRO MAX UNIVERSAL – Compatible AMD64 y ARM64 + Dashboard
...
function check_root() {
    [[ $EUID -ne 0 ]] && { echo -e "${ROJO}Debes ejecutar este script como root${RESET}"; exit 1; }
}
...
function detect_arch() {
    ...
    case $ARCH in
        ...
        *) echo -e "${ROJO}Arquitectura no soportada: $ARCH${RESET}"; exit 1;;
    esac
    echo -e "${VERDE}Arquitectura detectada: $MACHINE${RESET}"
    log "Arquitectura detectada: $MACHINE"
}

function check_installed() { [[ -x "$BIN" ]]; }
...
function install_badvpn() {
    ...
    apt install -y build-essential cmake git python3 python3-pip net-tools lsof >/dev/null 2>&1
    ...
    echo -e "${AMARILLO}Compilando BadVPN para $MACHINE...${RESET}"
    if [[ $MACHINE == "amd64" ]]; then
        cmake . -DBUILD_NOTHING_BY_DEFAULT=1 -DBUILD_UDPGW=1 >/dev/null 2>&1
    elif [[ $MACHINE == "arm64" ]]; then
        cmake . -DBUILD_NOTHING_BY_DEFAULT=1 -DBUILD_UDPGW=1 -DCMAKE_C_FLAGS="-march=armv8-a" >/dev/null 2>&1
    fi
    make >/dev/null 2>&1
